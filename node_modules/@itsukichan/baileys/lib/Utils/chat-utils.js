...
const processSyncAction = (syncAction, ev, me, initialSyncOpts, logger) => {
    ...
    else if (action?.chatLockSettingsAction) {
        ev.emit('chats.update', [
            {
                id,
                locked: !!action.chatLockSettingsAction.locked
            }
        ])
    }
    else if (action?.settingUpdateAction) {
        ev.emit('settings.update', {
            [type]: action.settingUpdateAction.value
        })
    }
    else {
